...
public:
    ...
    void setup(const int k, int a, int skip, bool double_precision = true, const T epsilon = 1e-5);
...
private:
     ...
     * @return  list of eigenvalues
     */
    std::vector<T> lanczos_eigen(int steps, const T epsilon = 1e-5);
     ...
     * @return  list of eigenvalues
     */
    std::vector<T> lanczos_no_spurious(SymTriMatrix<T> &tridiag, const T epsilon = 1e-5);
     ...
     * @return  list of eigenvalues
     */
    std::vector<T> tqlrat_eigen(const SymTriMatrix<T> &matrix, const T epsilon = 1e-5);
    /**
     ...
     * @return  list of eigenvalues
     */
    std::vector<T> qr_eigen(const SymTriMatrix<T> &matrix, const T epsilon = 1e-5);
  ...
  template<class Matrix, typename T> Lanczos<Matrix, T> :: Lanczos(const Matrix &A) {
    ...
    template<class Matrix, typename T> void Lanczos<Matrix, T> :: setup(const int k, int a, int skip, bool double_precision, const T epsilon) {
  ...
  template<class Matrix, typename T> inline T Lanczos<Matrix, T>::getEigenvalue(int i) {
    ...
    template<class Matrix, typename T> void Lanczos<Matrix, T>::computekEigenvectors() {
        ...
        for (int t = 0; t < eigenvectors.size(); ++t) {
            if (eigenvectors[t].getDimension() <= 0) {
  ...
  /* TODO: getBasisCSRMatrix() */
    ...
    template <class Matrix, typename T> std::vector<T> Lanczos<Matrix, T> :: lanczos_eigen(int steps, const T epsilon) {
        ...
        this->steps = steps;  
        std::cout << "CPU Lanczos iterations: " << this->steps << std::endl;
        std::vector<T> e = lanczos_no_spurious(tridiag, k);
        return e;
        // return lanczos_no_spurious(tridiag, k, epsilon);
}
  ...
  Vector Lanczos<Matrix,T> :: inverse_iteration(T eigenvalue) {
    ...
    do {
        ...
        // TODO: must solve for tridiagonal matrix instead!!!
        // TODO: then use basis from Lanczos to compute eigenvectors of A. (cheaper)
